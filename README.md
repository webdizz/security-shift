...
`docker run --name=tiger -v `pwd`:/tmp/reports:rw --network host -e project_name=ropeytasks -e environment=local -e host=host.docker.internal -e port=8080 -e protocol=http --rm TODO -s basic`
